...
sed -i "s/hex_coded_pk: ''/hex_coded_pk: $HEX/g" $HOME/huggingmodel${CHOICE}/worker/config.yaml

cd $HOME/huggingmodel${CHOICE}/worker
execute_with_prompt 'allocmd generate worker --env prod --network allora-testnet-1'
execute_with_prompt 'sudo chmod -R +rx ./data/scripts'
